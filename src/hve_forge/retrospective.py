...
async def _run_retrospective(
    ...
    *,
    ...
    try:
        ...
        if error_message:
            console.print(f"\n[bold red]Error during retrospective:[/bold red] {error_message[0]}")
...
def retrospective(
    ...
    console.print(Panel("[bold]Starting retrospective analysis…[/bold]", border_style="magenta", expand=False))
    asyncio.run(_run_retrospective(effective_prompt, model, github_token, verbose=verbose))
